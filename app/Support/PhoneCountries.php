...
{
    /**
     * Full dial-code list (ISO 3166-1 alpha-2), Burkina Faso first.
     *
     * @return list<array{code: string, name: string, dial: string, flag: string}>
     */
    public static function all(): array
    {
        $countries = [
            ...
            ['code' => 'SX', 'name' => 'Saint-Martin (NL)', 'dial' => '+1721'],
        ...
        $withFlags = array_map(
            static fn (array $c): array => [
                ...$c,
                'flag' => self::emojiFlag($c['code']),
            ],
            $countries,
        );

        usort($withFlags, static function (array $a, array $b): int {
            if ($a['code'] === 'BF') {
                return -1;
            }
            if ($b['code'] === 'BF') {
    ...
    private static function emojiFlag(string $countryCode): string
    {
        $code = strtoupper($countryCode);
        if (strlen($code) !== 2 || $code === 'XK') {
            return '🏳️';
        }

        $chars = [];
        foreach (str_split($code) as $char) {
            $chars[] = mb_chr(0x1F1E6 + ord($char) - ord('A'));
        }
