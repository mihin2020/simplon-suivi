...
const form = useForm({
    ...
    started_at: toDate(props.cohort.started_at),
    ended_at:   toDate(props.cohort.ended_at),
    status:     props.cohort.status,
})

const submit = () => {
    form.put(`/campus/cohorts/${props.cohort.id}`)
}
</script>
...
<template>
    <div class="max-w-[720px] mx-auto space-y-xl">
        ...
        <div class="flex items-center gap-md">
            ...
            <div>
                <h1 class="text-h1 font-bold text-on-surface">Modifier la cohorte</h1>
                <p class="text-body-md text-secondary mt-xs">{{ cohort.name }}</p>
            </div>
        </div>
